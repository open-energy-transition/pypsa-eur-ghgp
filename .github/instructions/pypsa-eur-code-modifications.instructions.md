...
In myopic mode, nuclear powerplants are represented both as Generators (from PPM) and as Links (from add_existing_baseyear.py), but the Link is always idle because the 'EU uranium' bus has no supply. This leads to double-counting in installed capacity statistics.

### Fix

A guard was added in add_power_capacities_installed_before_baseyear() to skip nuclear Links entirely:
...
| File | Change |
|---|---|
| `scripts/add_existing_baseyear.py` | Added `continue` guard in `add_power_capacities_installed_before_baseyear()` skipping nuclear Links |

### Notes
- The bug exists in upstream PyPSA-Eur as well (see config.default.yaml). The fix is local to the GHGP project.
- Removing the nuclear Links does not affect dispatch or costs, only installed capacity statistics.

---

## 5 The `copy_cost_data_for_backcasting` Workflow

### Motivation

The upstream technology cost database (technology-data v0.14.0) provides cost files only for milestone years (e.g., 2020, 2025, 2030). For intermediate years (2021–2024), no cost files exist. However, PyPSA-Eur expects a cost file named `costs_{planning_horizons[0]}.csv` for each scenario year. To enable backcasting for these years, a workflow is needed to generate the required cost files.
...
- The rule is only active when `backcasting.enable: true` in the config.
- It copies the nearest available milestone cost file (e.g., `costs_2025.csv`) to the required intermediate year (e.g., `costs_2024.csv`).
- The rule takes precedence over the default `retrieve_cost_data` rule for the same output file.

### Effect on the Workflow

- When a scenario requires `costs_2024.csv`, Snakemake will:
  1. Use `copy_cost_data_for_backcasting` to produce `costs_2024.csv` by copying `costs_2025.csv`.
  2. Use the copied file as input to `process_cost_data`, which generates `costs_2024_processed.csv`.
- This ensures that all required cost files exist for any backcasting year, even if not present in the upstream archive.
- The actual fuel prices for each year are still set via `data/custom_costs.csv` overrides, so the main effect is to provide a valid cost structure for PyPSA-Eur to run.

### Summary Table

| Rule | Input | Output | Condition |
|---|---|---|---|
| `copy_cost_data_for_backcasting` | `costs_{year_costs}.csv` | `costs_{planning_horizons}.csv` | `backcasting.enable: true` |
